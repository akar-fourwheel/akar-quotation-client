...
export default function BookTestDrivePage() {
  ...
  useEffect(() => {
    Promise.all([
      axios.get("/customer-info", {
        params: { role: DEFAULT_ROLE, name: DEFAULT_CA_ID },
      }),
      axios.get("/test-drive/get-demo-vehicles"),
    ])
      .then(([customersRes, vehiclesRes]) => {
        setCustomerList(customersRes.data.data || []);
        setDemoVehicles(vehiclesRes.data.rows || []);
      })
      .catch(() => {
        setMsg({ text: "Failed to fetch data", ok: false });
      })
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
        ...
        if (name === "cxID") {
        const selectedCustomer = customerList.find(c => String(c.id) === value);
        updated.alotID = selectedCustomer?.allotment_id || "";
        }

        if (name === "model") updated.variant = "";
  ...
  const handleDateChange = (date) => {
    const time = form.scheduledDateTime?.split("T")[1] || "00:00";
    setForm((prev) => ({
      ...prev,
  ...
  const handleTimeChange = (time) => {
    const date =
      form.scheduledDateTime?.split("T")[0] ||
  ...
  const validate = () => {
    ...
    if (!form.alotID) e.alotID = "Allotment ID missing for this customer";
  ...
  // The return statement (JSX/UI) is unmodified below
  return (
                  ...
                  <div className="lg:col-span-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <input
                        ...
                        className={`w-full sm:w-1/2 px-4 py-3 border rounded-lg ${errors.scheduledDateTime ? "border-red-500" : "border-gray-300"} focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors`}
                      />
                      <input
                        type="time"
